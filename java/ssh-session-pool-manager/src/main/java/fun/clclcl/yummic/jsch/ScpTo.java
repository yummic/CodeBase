package fun.clclcl.yummic.jsch;

import com.jcraft.jsch.*;
import fun.clclcl.yummic.jsch.pool.SshConfig;
import fun.clclcl.yummic.jsch.pool.SshSessionManager;

import java.io.*;

public class ScpTo {

    public static void copy(String lfile, String rfile, boolean ptimestamp, SshSessionManager sshManager, SshConfig sshConfig) throws IOException {
        FileInputStream fis=null;
        Session session = null;
        try {
            session = sshManager.getSession(sshConfig);

            // exec 'scp -t rfile' remotely
            rfile=rfile.replace("'", "'\"'\"'");
            rfile="'"+rfile+"'";
            String command="scp " + (ptimestamp ? "-p" :"") +" -t "+rfile;
            Channel channel=session.openChannel("exec");
            ((ChannelExec)channel).setCommand(command);

            // get I/O streams for remote scp
            OutputStream out=channel.getOutputStream();
            InputStream in=channel.getInputStream();
            channel.connect();

            if(checkAck(in) != 0){
                throw new IOException("Copy file failed.");
            }

            File _lfile = new File(lfile);

            if (ptimestamp) {
                command="T"+(_lfile.lastModified()/1000)+" 0";
                // The access time should be sent here,
                // but it is not accessible with JavaAPI ;-<
                command+=(" "+(_lfile.lastModified()/1000)+" 0\n");
                out.write(command.getBytes()); out.flush();
                if(checkAck(in) != 0){
                    throw new IOException("Copy file failed.");
                }
            }

            // send "C0644 filesize filename", where filename should not include '/'
            long filesize=_lfile.length();
            command="C0644 "+filesize+" ";
            if(lfile.lastIndexOf('/')>0){
                command+=lfile.substring(lfile.lastIndexOf('/')+1);
            }
            else{
                command+=lfile;
            }
            command+="\n";
            out.write(command.getBytes());
            out.flush();
            if(checkAck(in) != 0){
                throw new IOException("Copy file failed.");
            }

            // send a content of lfile
            fis=new FileInputStream(lfile);
            byte[] buf=new byte[1024];
            while(true){
                int len=fis.read(buf, 0, buf.length);
                if(len<=0) break;
                out.write(buf, 0, len); //out.flush();
            }
            fis.close();
            fis=null;
            // send '\0'
            buf[0] = 0;
            out.write(buf, 0, 1);
            out.flush();
            if(checkAck(in) != 0){
                throw new IOException("Copy file failed.");
            }
            out.close();
            channel.disconnect();
        } catch (Exception e) {
            try {
                if(fis != null) {
                    fis.close();
                }
            } catch (Exception ee) {
                //ignore error.
            }
            throw new RuntimeException("Error occurs while scp file.", e);
        } finally {
            sshManager.returnSession(sshConfig.getHost(), session);
        }
    }

    static int checkAck(InputStream in) throws IOException{
        int b = in.read();
        // b may be 0 for success,
        //          1 for error,
        //          2 for fatal error,
        //          -1
        if(b == 0) return b;
        if(b == -1) return b;

        if(b == 1 || b == 2) {
            StringBuffer sb=new StringBuffer();
            int c;
            do {
                c = in.read();
                sb.append((char)c);
            } while (c != '\n');
            if(b == 1 || b == 2){ // 1 error, 2 fatal error
                throw new RuntimeException("Error occurs while scp file : " + sb.toString());
            }
        }
        return b;
    }

}